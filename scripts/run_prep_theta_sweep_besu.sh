...
RUN_ID=$(date +%Y%m%d_%H%M%S)_prep_theta_sweep_besu
...
# ── Main loop ─────────────────────────────────────────────────────────────────
TOTAL_RUNS=$(( 1 + REPLICATIONS * 2 * 3 ))
echo "======================================================================"
echo "PREP theta_p sweep Besu | 150 TPS | 120s+300s | ${TOTAL_RUNS} runs total"
echo "Run ID: ${RUN_ID}"
...
echo "======================================================================"
echo ""
echo "Per-theta deferral + GC summary:"
for theta in ${THETA_VALUES}; do
    for v in prep prep_sched; do
        echo "  ${v}_theta${theta}:"
        for rep in $(seq 1 ${REPLICATIONS}); do
            f="${RESULTS_DIR}/${v}_theta${theta}_${rep}/gc_summary.txt"
            if [ -f "${f}" ]; then
                echo "    rep${rep}: $(grep total_gc_ms "$f") $(grep prep_deferred "$f") $(grep avg_tx_per_block "$f")"
            else
                echo "    rep${rep}: NO SUMMARY (failed, check ${RESULTS_DIR}/${v}_theta${theta}_${rep}/)"
            fi
        done
    done
done
