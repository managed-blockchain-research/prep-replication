...
echo ""
echo "Resuming — 19 runs remaining out of 25 (5 variants × 5 reps, interleaved)..."
echo ""

VARIANTS=("baseline:DISABLED" "last_hfl:HFL_5_5" "mats:MATS" "prep:PREP" "prep_sched:PREP_SCHED")

# Confirmed clean from the original run — verified via gc_summary.txt trace_duration_s
# in the healthy 294-399s range (not the anomalous/collapsed pattern that flagged the
# other rep-2 labels for redo).
SKIP_LABELS=("baseline_1" "last_hfl_1" "mats_1" "prep_1" "prep_sched_1" "baseline_2")
...
for rep in $(seq 1 "${REPLICATIONS}"); do
    ...
    for var_spec in "${VARIANTS[@]}"; do
        ...
        if is_skip_label "${local_label}"; then
            echo ""
            echo "  ⏭  ${local_label} already clean from prior run — skipping"
            continue
        fi
