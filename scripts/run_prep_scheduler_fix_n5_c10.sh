...
Run ID: ${RUN_ID}
Date:   $(date)
Host:   $(hostname) [compute10 half: PREP, PREP_SCHED -- compute23 covers Baseline, HFL, MATS]
...
Variants (this host): PREP, PREP_SCHED -- n=${REPLICATIONS} each
...
dropped ~3.6-4.1x, within-run throughput retention vs baseline rose from
pre-fix ~57%/52% to ~72%/70% for PREP/PREP_SCHED respectively. This n=5
run confirms whether that holds on a clean host.
...
# ── Main loop (compute10 half: PREP, PREP_SCHED, n=5) ───────────────────────────
TOTAL_RUNS=$(( REPLICATIONS * 2 ))
echo "======================================================================"
echo "Scheduler fix n=5 [compute10: PREP/PREP_SCHED] | 150 TPS | 120s+300s | ${TOTAL_RUNS} runs"
echo "Run ID: ${RUN_ID}"
...
echo ""
echo "======================================================================"
echo "compute10 half: all ${TOTAL_RUNS} runs complete."
echo "Results: ${RESULTS_DIR}"
echo "======================================================================"
echo ""
echo "Summary:"
for v in prep prep_sched; do
    echo "  ${v}:"
    for rep in $(seq 1 ${REPLICATIONS}); do
