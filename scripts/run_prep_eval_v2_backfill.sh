...
# The 4 labels that need a redo: prep_3 failed at contract-deploy (transient
# timeout, unrelated to the zombie-worker bug or host contention). prep_sched_2/4/5
# all showed the host-contention collapse signature (trace_duration_s=0.3s, ~420-480
# blocks vs thousands for healthy siblings) confirmed via per-run data-value checks
# during the main resume run. Everything else in the 25 is already clean.
RUN_ONLY_LABELS=("prep_3" "prep_sched_2" "prep_sched_4" "prep_sched_5")
