...
run_single() {
    ...
    # Unconditional: workers have leaked even when caliper_exit=0 (e.g. a
    # short-trace collapse caught by the health-check below, not by the exit
    # code), so gating this on a non-zero exit missed exactly that case.
    pkill -9 -f "caliper launch" 2>/dev/null || true
...
VARIANTS=("baseline:DISABLED" "last_hfl:HFL_5_5" "mats:MATS" "prep_sched:PREP_SCHED" "prep:PREP")
...
RUN_ONLY_LABELS=("prep_sched_1")
# Note: mats_1/prep_1 keep their original rep1 data (genuine high-GC anomaly,
# not a harness bug -- confirmed unaffected by the trace_duration_s fix, see
# gc_summary.txt: 74-87 total GC events vs 2-8 for sibling reps). Only
# prep_sched_1 is redone here, because its original last_metrics.csv was
# independently found corrupted (torn/interleaved writes, inconsistent NF).
...
IDLE_CORES_MIN=38          # load1 < 10 on this 48-core host -- relaxed from load1<4:
                           # collapses reproduced even at idle_cores=44-46 (load1~2-3)
                           # this session, so the strict gate wasn't actually predictive;
                           # this just avoids waiting out normal office-hours load for no reason.
...
run_single_with_retry() {
    ...
    while true; do
        ...
        if [ -f "${run_dir}/gc_summary.txt" ]; then
            local tdur blocks csv_ok
            tdur=$(grep "trace_duration_s" "${run_dir}/gc_summary.txt" | cut -d= -f2)
            blocks=$(tail -n +2 "${run_dir}/last_metrics.csv" 2>/dev/null | wc -l)
            # trace_duration_s is first-GC-event-to-last-GC-event span, NOT wall-clock
            # duration -- it craters toward 0 when few GC events happen to land close
            # together in time, which is common precisely when a variant is doing its
            # job of reducing GC pressure. It is not a valid liveness signal; block
            # count (and CSV integrity) is.
            csv_ok=$(awk -F',' 'NR==1{n=NF; next} NF!=n{bad=1} END{print (bad?"0":"1")}' "${run_dir}/last_metrics.csv" 2>/dev/null)
            if [ "${blocks:-0}" -ge "${HEALTHY_BLOCKS_MIN}" ] && [ "${csv_ok:-0}" = "1" ]; then
                echo "  ✓ ${label} healthy (tdur=${tdur}, blocks=${blocks}) — accepted"
                return 0
            fi
            echo "  ⚠ ${label} attempt ${attempt} looks like a collapse (tdur=${tdur}, blocks=${blocks}, csv_ok=${csv_ok:-0}) — retrying"
        else
