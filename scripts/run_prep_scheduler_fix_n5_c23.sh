...
#
# SPLIT n=5 CONFIRMATORY RUN (compute23 half): n=1 rough check (contaminated
# by another user's job on the host it ran on) already showed a real signal
# -- scheduler_overhead_us dropped ~4x and within-run throughput retention
# vs baseline rose from ~43-57% (pre-fix) to ~70-72% (post-fix). This n=5
# run gets a clean, publication-grade dataset. compute23's theta_p sweep
# (using the ORIGINAL unpatched ~/besu-mats) has finished, so this host is
# now free and safe to use with the shared, non-isolated caliper-stress-test
# dir directly (no other host writing there concurrently in this run).
# This half covers Baseline, HFL, MATS (compute10 covers PREP, PREP_SCHED).
...
RUN_ID="20260831_071500_prep_scheduler_fix_n5"
RESULTS_DIR="/home/yeochan.yoon/caliper-stress-test/results/prep_scheduler_fix_n5/${RUN_ID}"
mkdir -p "${RESULTS_DIR}"
...
LASTScheduler.reorder() Performance-Fix n=5 CONFIRMATORY RUN — Besu 4 GB / 150 TPS
=============================================================
Run ID: ${RUN_ID}
Date:   $(date)
Host:   $(hostname) [compute23 half: Baseline, HFL, MATS -- compute10 covers PREP, PREP_SCHED]
Binary: ${BESU_BIN} (FIXED jar, isolated install; canonical unpatched
        install at ~/besu-mats untouched)
Heap:   -Xms4g -Xmx4g (G1GC, MaxGCPauseMillis=200)
Variants (this host): DISABLED, HYBRID_FEE_LOCALITY(0.5/0.5), MATS -- n=${REPLICATIONS} each
...
n=1 rough check (contaminated host) already showed: scheduler_overhead_us
dropped ~3.6-4.5x (109-136k us pre-fix -> ~29.5-33.3k us), within-run
throughput retention vs baseline rose from pre-fix ~43-57% to ~70-72%.
This n=5 run confirms whether that holds on a clean host.

Canonical (pre-fix, n=5) reference values:
  Baseline:   34.0 tx/s,  270.8 tx/block, 66.6 ms/1000tx, sched_us~0
  HFL:        14.6 tx/s,   40.6 tx/block, 27.8 ms/1000tx, sched_us~129000
  MATS:       14.7 tx/s,   42.4 tx/block, 26.9 ms/1000tx, sched_us~132000
  PREP:       19.5 tx/s,   38.4 tx/block, 26.3 ms/1000tx, sched_us~109000
  PREP_SCHED: 17.8 tx/s,   40.3 tx/block, 26.7 ms/1000tx, sched_us~136000
EOF

# ── Main loop (compute23 half: Baseline, HFL, MATS, n=5) ────────────────────────
TOTAL_RUNS=$(( REPLICATIONS * 3 ))
echo "======================================================================"
echo "Scheduler fix n=5 [compute23: Baseline/HFL/MATS] | 150 TPS | 120s+300s | ${TOTAL_RUNS} runs"
echo "Run ID: ${RUN_ID}"
...
echo ""
echo "======================================================================"
echo "compute23 half: all ${TOTAL_RUNS} runs complete."
echo "Results: ${RESULTS_DIR}"
echo "======================================================================"
echo ""
echo "Summary:"
for v in baseline last_hfl mats; do
    echo "  ${v}:"
    for rep in $(seq 1 ${REPLICATIONS}); do
        f="${RESULTS_DIR}/${v}_${rep}/gc_summary.txt"
        if [ -f "${f}" ]; then
            echo "    rep${rep}: $(grep tx_per_sec "$f") $(grep gc_ms_per_1000tx "$f") $(grep scheduler_overhead "$f")"
        else
            echo "    rep${rep}: NO SUMMARY (failed, check ${RESULTS_DIR}/${v}_${rep}/)"
        fi
    done
done
